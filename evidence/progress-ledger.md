...
| E.protocol-negotiation | pass | native `capabilities`; MCP initialize; `agent-negotiation`, `mcp-protocol` | Grants, revision, limits, Triton protocol, and MCP version are machine-readable; framing is bounded | Add per-tool mutation envelope schemas |
